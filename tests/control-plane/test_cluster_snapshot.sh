...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-cluster-snapshot.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_cluster_snapshot.c" \
    -o "$tmpdir/test_cluster_snapshot"
"$tmpdir/test_cluster_snapshot"
