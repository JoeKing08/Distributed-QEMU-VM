...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-capability-record.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_capability_record.c" \
    -o "$tmpdir/test_capability_record"
"$tmpdir/test_capability_record"
