...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-vcpu-handoff.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_vcpu_handoff.c" \
    -o "$tmpdir/test_vcpu_handoff"
"$tmpdir/test_vcpu_handoff"
