...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-membership-route.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_membership_route.c" \
    -o "$tmpdir/test_membership_route"
"$tmpdir/test_membership_route"
