...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-route-delivery.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_route_delivery.c" \
    -o "$tmpdir/test_route_delivery"
"$tmpdir/test_route_delivery"
