...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-membership-controller.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_membership_controller.c" \
    -pthread -o "$tmpdir/test_membership_controller"
"$tmpdir/test_membership_controller"
