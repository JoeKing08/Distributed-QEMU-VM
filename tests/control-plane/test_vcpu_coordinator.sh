...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-vcpu-coordinator.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_vcpu_coordinator.c" \
    -pthread -o "$tmpdir/test_vcpu_coordinator"
"$tmpdir/test_vcpu_coordinator"
