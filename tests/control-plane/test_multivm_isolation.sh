...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-multivm-isolation.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_multivm_isolation.c" \
    -o "$tmpdir/test_multivm_isolation"
"$tmpdir/test_multivm_isolation"
