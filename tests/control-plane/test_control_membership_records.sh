...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-control-membership-records.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_control_membership_records.c" \
    -o "$tmpdir/test_control_membership_records"
"$tmpdir/test_control_membership_records"
