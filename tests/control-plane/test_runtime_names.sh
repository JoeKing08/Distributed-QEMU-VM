...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-runtime-names.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_runtime_names.c" \
    -o "$tmpdir/test_runtime_names"
"$tmpdir/test_runtime_names"
