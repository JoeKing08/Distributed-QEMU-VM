...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-route-runtime.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_route_runtime.c" \
    -pthread -o "$tmpdir/test_route_runtime"
"$tmpdir/test_route_runtime"
