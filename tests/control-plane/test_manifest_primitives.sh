...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-manifest-primitives.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_manifest_primitives.c" \
    -o "$tmpdir/test_manifest_primitives"
"$tmpdir/test_manifest_primitives"
