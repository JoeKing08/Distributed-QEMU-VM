...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-control-transport.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_control_transport.c" \
    -pthread -o "$tmpdir/test_control_transport"
"$tmpdir/test_control_transport"
