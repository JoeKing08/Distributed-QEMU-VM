...
#include "wavevm_local_memory_v1.h"

static int expect(int condition, const char *message)
{
    if (!condition) {
        fprintf(stderr, "local-memory-v1 test: %s\n", message);
        return -1;
    }
    return 0;
}
...
int main(void)
{
    ...
    struct wvm_local_memory_v1_fault_request decoded;
    struct wvm_local_memory_v1_commit_request commit_request;
    struct wvm_local_memory_v1_commit_request decoded_commit_request;
    struct wvm_local_memory_v1_commit_result commit_result;
    struct wvm_local_memory_v1_commit_result decoded_commit_result;
    uint8_t encoded[WVM_LOCAL_MEMORY_V1_FAULT_REQUEST_BYTES];
    uint8_t commit_encoded[WVM_LOCAL_MEMORY_V1_COMMIT_REQUEST_HEADER_BYTES +
                           8U];
    uint8_t commit_result_encoded[WVM_LOCAL_MEMORY_V1_COMMIT_RESULT_BYTES];
    uint8_t commit_data[3] = {0x31, 0x32, 0x33};
    uint8_t length[WVM_LOCAL_MEMORY_V1_RESULT_LENGTH_BYTES];
    ...
    request.gpa = 0x4000;
    if (expect(wvm_local_memory_v1_fault_request_encode(
                   &request, encoded, error, sizeof(error)) == 0 &&
                   wvm_local_memory_v1_fault_request_decode(
        ...
        expect(wvm_local_memory_v1_result_length_encode(
                   WVM_V1_MEM_ACK_HEADER_BYTES + WVM_V1_MEMORY_PAGE_BYTES,
    ...
    if (expect(wvm_local_memory_v1_commit_request_encode(
                   &commit_request, commit_encoded, sizeof(commit_encoded),
                   &commit_encoded_bytes, error, sizeof(error)) == 0 &&
                   wvm_local_memory_v1_commit_request_decode(
    ...
    commit_result.ack.status = WVM_V1_MEM_COMMIT_ACK_SUCCESS;
    commit_result.ack.directory_physical_node_id = 9;
    commit_result.ack.directory_node_instance_id = 10;
    if (expect(wvm_local_memory_v1_commit_result_encode(
    ...
    if (expect(wvm_local_memory_v1_fault_request_encode(
        ...
        expect(wvm_local_memory_v1_result_length_encode(
                   WVM_V1_MEM_ACK_HEADER_BYTES + 1U, length, error,
                   sizeof(error)) != 0,
    ...
    puts("local-memory-v1 tests: PASS");
    return 0;
}
