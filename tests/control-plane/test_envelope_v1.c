...
{
    if (!condition) {
        fprintf(stderr, "envelope-v1 test: %s\n", message);
        return -1;
    }
    return 0;
}
...
struct emitted_frames {
    uint8_t frames[8][WVM_ENVELOPE_V1_MAX_NETWORK_FRAME_BYTES];
...
static void fill_routed_envelope(struct wvm_envelope_v1 *envelope,
...
int main(void)
{
    struct wvm_envelope_v1 input;
    struct wvm_envelope_v1 decoded;
    struct wvm_envelope_v1_admitted_identity admitted;
    struct wvm_envelope_v1_reassembler reassembler;
    struct wvm_envelope_v1_reassembled completed;
    uint8_t payload[] = {0x10, 0x20, 0x30, 0x40};
    uint8_t oversized[WVM_ENVELOPE_V1_MAX_NETWORK_FRAME_BYTES];
    uint8_t encoded[WVM_ENVELOPE_V1_HEADER_BYTES +
                    WVM_ENVELOPE_V1_ROUTE_PREFIX_BYTES + sizeof(payload)];
    uint8_t logical[1500];
    uint8_t memory_ack_logical[32 + 4096];
    uint8_t fragment_zero[WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES + 1024];
    uint8_t fragment_one[WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES + 476];
    ...
    if (expect(wvm_envelope_v1_crc32c((const uint8_t *)"123456789", 9) ==
                   0xe3069283U,
               "CRC32C Castagnoli vector") ||
        expect(wvm_envelope_v1_encode(
                   &input, WVM_ENVELOPE_V1_TRANSPORT_NETWORK, encoded,
        ...
        expect(wvm_envelope_v1_decode(
                   encoded, encoded_bytes, WVM_ENVELOPE_V1_TRANSPORT_NETWORK,
    ...
    if (expect(wvm_envelope_v1_validate_admitted(&decoded, &admitted, error,
                                                 sizeof(error)) == 0,
               "admitted identity") ||
        expect((admitted.vm_incarnation++,
                wvm_envelope_v1_validate_admitted(
    ...
    if (expect(wvm_envelope_v1_encode(
                   &input, WVM_ENVELOPE_V1_TRANSPORT_NETWORK, encoded,
                   sizeof(encoded), &encoded_bytes, error, sizeof(error)) == 0 &&
                   wvm_envelope_v1_decode(
                       encoded, encoded_bytes,
                       WVM_ENVELOPE_V1_TRANSPORT_NETWORK, &decoded, error,
                       sizeof(error)) == 0 &&
                   decoded.route.destination_scope == 0x1234 &&
                   wvm_envelope_v1_route_advance(&decoded, error,
                                                  sizeof(error)) == 0 &&
    ...
    input.message_type = WVM_ENVELOPE_V1_MSG_MEM_READ;
    memset(oversized, 0, sizeof(oversized));
    input.payload = oversized;
    input.payload_bytes = sizeof(oversized);
    if (expect(wvm_envelope_v1_encode(
                   &input, WVM_ENVELOPE_V1_TRANSPORT_NETWORK, encoded_fragment,
    ...
    encoded[4] = 0;
    encoded[5] = 2;
    if (expect(wvm_envelope_v1_decode(
                   encoded, encoded_bytes, WVM_ENVELOPE_V1_TRANSPORT_NETWORK,
                   &decoded, error, sizeof(error)) != 0,
    ...
    encoded[WVM_ENVELOPE_V1_HEADER_BYTES] ^= 0xff;
    if (expect(wvm_envelope_v1_decode(
                   encoded, encoded_bytes, WVM_ENVELOPE_V1_TRANSPORT_NETWORK,
                   &decoded, error, sizeof(error)) != 0,
               "reject bad CRC")) {
    ...
    input.flags = WVM_ENVELOPE_V1_FLAG_FRAGMENTED;
    wvm_envelope_v1_semantic_digest(logical, sizeof(logical),
    ...
    fragment_zero[12] = 0;
    if (expect(wvm_envelope_v1_reassembler_init(&reassembler) == 0,
        ...
        expect(wvm_envelope_v1_encode(
                   ...
                   wvm_envelope_v1_decode(
                       ...
                       WVM_ENVELOPE_V1_TRANSPORT_NETWORK, &decoded, error,
                       sizeof(error)) == 0 &&
                   wvm_envelope_v1_reassembler_accept(
                       &reassembler, &decoded, 100, &completed, error,
                       sizeof(error)) == 0,
               "accept first fragment") ||
        expect(wvm_envelope_v1_reassembler_accept(
    ...
    if (expect(wvm_envelope_v1_encode(
        ...
        expect(completed.payload_bytes == sizeof(logical) &&
                   ...
                   !(completed.envelope.flags &
                     WVM_ENVELOPE_V1_FLAG_FRAGMENTED),
               "reassembled semantic payload")) {
        wvm_envelope_v1_reassembled_release(&completed);
        wvm_envelope_v1_reassembler_destroy(&reassembler);
        return 1;
    }
    wvm_envelope_v1_reassembled_release(&completed);
    ...
    }
    {
        ...
        if (expect(wvm_envelope_v1_emit_network_frames(
                       &input, collect_frame, &emitted, error,
                       sizeof(error)) == 0 &&
                       emitted.count == 5,
                   "emit bounded frames for a full memory ACK") ||
            expect(wvm_envelope_v1_reassembler_init(&reassembler) == 0,
        ...
        for (i = 0; i < emitted.count; i++) {
            ...
            if (i + 1U == emitted.count) {
                if (expect(wvm_envelope_v1_reassembler_accept(
            ...
            } else if (expect(wvm_envelope_v1_reassembler_accept(
