...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-admission-plan.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_admission_plan.c" \
    -o "$tmpdir/test_admission_plan"
"$tmpdir/test_admission_plan"
