...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-envelope-v1.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_envelope_v1.c" \
    -pthread -o "$tmpdir/test_envelope_v1"
"$tmpdir/test_envelope_v1"
