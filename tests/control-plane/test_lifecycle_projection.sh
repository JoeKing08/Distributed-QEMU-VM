...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-lifecycle-projection.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_lifecycle_projection.c" \
    -o "$tmpdir/test_lifecycle_projection"
"$tmpdir/test_lifecycle_projection"
