...
struct wvm_v1_directory_page {
    ...
    struct wvm_v1_directory_page *next;
};
...
{
    uint64_t page = gpa / WVM_V1_MEMORY_PAGE_BYTES;

    page ^= page >> 33;
    page *= UINT64_C(0xff51afd7ed558ccd);
    page ^= page >> 33;
    return (size_t)page & (WVM_V1_DIRECTORY_STORE_BUCKETS - 1U);
}

static struct wvm_v1_directory_page *find_page_locked(
    struct wvm_v1_directory_store *store, size_t bucket, uint64_t gpa)
{
    struct wvm_v1_directory_page *page;
    ...
    size_t error_len)
{
    ...
    store->config = *config;
    store->buckets = calloc(WVM_V1_DIRECTORY_STORE_BUCKETS,
                            sizeof(*store->buckets));
    store->bucket_locks = calloc(WVM_V1_DIRECTORY_STORE_BUCKETS,
    ...
    for (i = 0; i < WVM_V1_DIRECTORY_STORE_BUCKETS; i++) {
...
void wvm_v1_directory_store_destroy(struct wvm_v1_directory_store *store)
{
    ...
    for (i = 0; i < WVM_V1_DIRECTORY_STORE_BUCKETS; i++) {
        struct wvm_v1_directory_page *page = store->buckets[i];

        while (page) {
            struct wvm_v1_directory_page *next = page->next;
