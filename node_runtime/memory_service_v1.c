...
enum pending_entry_state {
    WVM_V1_MEMORY_PENDING_EMPTY = 0,
    WVM_V1_MEMORY_PENDING_WAITING = 1,
    WVM_V1_MEMORY_PENDING_COMPLETED = 2,
};

enum local_waiter_state {
    WVM_V1_MEMORY_WAITER_EMPTY = 0,
    WVM_V1_MEMORY_WAITER_WAITING = 1,
    WVM_V1_MEMORY_WAITER_COMPLETED = 2,
    WVM_V1_MEMORY_WAITER_ORPHANED = 3,
};
...
struct global_memory_service {
    ...
    struct local_waiter_entry waiters[WVM_V1_MEMORY_SERVICE_MAX_PENDING];
    struct local_commit_waiter_entry
        commit_waiters[WVM_V1_MEMORY_SERVICE_MAX_PENDING];
};
...
enum local_commit_waiter_state {
    WVM_V1_MEMORY_COMMIT_WAITER_EMPTY = 0,
    WVM_V1_MEMORY_COMMIT_WAITER_WAITING = 1,
    WVM_V1_MEMORY_COMMIT_WAITER_COMPLETED = 2,
    WVM_V1_MEMORY_COMMIT_WAITER_ORPHANED = 3,
};
...
static uint64_t retention_deadline_ms(
    const struct wvm_v1_memory_service *service)
{
    ...
    char *error, size_t error_len)
{
    ...
    wvm_envelope_v1_semantic_digest(payload, payload_bytes, digest);
    return 0;
}

static int route_destination_equal(
    const struct wvm_runtime_route_destination *destination,
    const struct wvm_envelope_v1_route *route)
{
...
static struct wvm_v1_memory_pending_entry *find_pending_locked(
    struct wvm_v1_memory_service *service,
    const uint8_t operation_id[WVM_IDENTITY_ID_BYTES])
{
    size_t i;

    for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
        struct wvm_v1_memory_pending_entry *entry = &service->pending[i];

        if (entry->state != WVM_V1_MEMORY_PENDING_EMPTY &&
...
static int pending_register(struct wvm_v1_memory_service *service,
                            const uint8_t operation_id[WVM_IDENTITY_ID_BYTES],
                            const struct wvm_runtime_memory_dispatch *memory,
                            uint64_t gpa, int *already_waiting)
{
    struct wvm_v1_memory_pending_entry *entry;
    size_t i;
    ...
    if (entry) {
        ...
        *already_waiting =
            entry->state == WVM_V1_MEMORY_PENDING_WAITING;
        pthread_mutex_unlock(&service->pending_lock);
...
static void pending_remove(struct wvm_v1_memory_service *service,
                           const uint8_t operation_id[WVM_IDENTITY_ID_BYTES])
{
    struct wvm_v1_memory_pending_entry *entry;
    ...
    pthread_mutex_lock(&service->pending_lock);
    entry = find_pending_locked(service, operation_id);
    if (entry && entry->state == WVM_V1_MEMORY_PENDING_WAITING) {
        memset(entry, 0, sizeof(*entry));
    }
    pthread_mutex_unlock(&service->pending_lock);
}

static struct wvm_v1_memory_outgoing_commit_entry *
    ...
    const uint8_t operation_id[WVM_IDENTITY_ID_BYTES])
{
    ...
    for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
        struct wvm_v1_memory_outgoing_commit_entry *entry =
            &service->outgoing_commits[i];

        if (entry->state != WVM_V1_MEMORY_COMMIT_EMPTY &&
            memcmp(entry->operation_id, operation_id,
                   sizeof(entry->operation_id)) == 0) {
            return entry;
        }
...
{
    struct wvm_v1_memory_outgoing_commit_entry *empty = NULL;
    size_t i;
    ...
    for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
        ...
        if (entry->state == WVM_V1_MEMORY_COMMIT_IN_FLIGHT) {
            *already_waiting = 1;
            pthread_mutex_unlock(&service->outgoing_commit_lock);
            return 0;
        }
    ...
    empty->state = WVM_V1_MEMORY_COMMIT_IN_FLIGHT;
    *already_waiting = 0;
    pthread_mutex_unlock(&service->outgoing_commit_lock);
    return 0;
}
    ...
    const uint8_t operation_id[WVM_IDENTITY_ID_BYTES])
{
    ...
    if (entry && entry->state == WVM_V1_MEMORY_COMMIT_IN_FLIGHT) {
        memset(entry, 0, sizeof(*entry));
    }
    pthread_mutex_unlock(&service->outgoing_commit_lock);
}
...
static int resolve_route(
    const struct wvm_v1_memory_service *service,
    const struct wvm_runtime_route_destination *destination,
    struct wvm_envelope_v1_route *route, char *error, size_t error_len)
{
...
static void envelope_from_projection(
    const struct wvm_v1_memory_service *service, uint16_t message_type,
    const uint8_t operation_id[WVM_IDENTITY_ID_BYTES],
    uint64_t delivery_attempt_id, const struct wvm_envelope_v1_route *route,
...
static int complete_local_fault(struct wvm_v1_memory_service *service,
...
static int service_remote_read(struct wvm_v1_memory_service *service,
                               const struct wvm_envelope_v1 *envelope,
                               char *error, size_t error_len)
{
    ...
    if (wvm_v1_mem_read_decode(envelope->payload, envelope->payload_bytes,
    ...
    if (read_result == 0) {
        ...
        ack.status = read_result == -ENOENT ? WVM_V1_MEM_ACK_NOT_FOUND
                                            : WVM_V1_MEM_ACK_INTERNAL_FAILURE;
    }
    ...
    if (wvm_v1_mem_ack_envelope_build(
...
static int service_remote_ack(struct wvm_v1_memory_service *service,
                              const struct wvm_envelope_v1 *envelope,
                              char *error, size_t error_len)
{
    struct wvm_v1_mem_ack ack;
    struct wvm_v1_memory_pending_entry *pending;
    uint8_t payload_digest[WVM_SHA256_DIGEST_BYTES];
    int completion_result;

    if (wvm_v1_mem_ack_decode(envelope->payload, envelope->payload_bytes, &ack,
                              error, error_len) != 0 ||
        !route_destination_equal(&service->config.dispatch->local_primary,
                                 &envelope->route)) {
    ...
    wvm_envelope_v1_semantic_digest(envelope->payload, envelope->payload_bytes,
                                    payload_digest);
    pthread_mutex_lock(&service->pending_lock);
    ...
    if (pending->state == WVM_V1_MEMORY_PENDING_COMPLETED) {
        int duplicate =
    ...
    pthread_mutex_lock(&service->pending_lock);
    pending = find_pending_locked(service, envelope->operation_id);
    if (pending && pending->state == WVM_V1_MEMORY_PENDING_WAITING) {
        pending->state = WVM_V1_MEMORY_PENDING_COMPLETED;
...
static int commit_entry_begin(
    ...
    const struct wvm_envelope_v1 *envelope,
    const uint8_t semantic_payload_digest[WVM_SHA256_DIGEST_BYTES],
    struct wvm_v1_memory_commit_entry **entry_out, int *replay,
    char *error, size_t error_len)
{
    struct wvm_v1_memory_commit_entry *empty = NULL;
    size_t i;
    ...
    for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
        struct wvm_v1_memory_commit_entry *entry = &service->commits[i];
        ...
        if (entry->state == WVM_V1_MEMORY_COMMIT_COMPLETED) {
            *entry_out = entry;
            *replay = 1;
        } else {
            /*
             ...
             * a fresh attempt.
             */
            entry->state = WVM_V1_MEMORY_COMMIT_IN_FLIGHT;
            *entry_out = entry;
            *replay = 0;
        }
        pthread_mutex_unlock(&service->commit_lock);
...
static void commit_entry_complete(
    struct wvm_v1_memory_service *service,
    struct wvm_v1_memory_commit_entry *entry,
    const struct wvm_v1_mem_commit_ack *result)
{
    ...
    pthread_mutex_lock(&service->commit_lock);
    entry->result = *result;
    entry->state = WVM_V1_MEMORY_COMMIT_COMPLETED;
    pthread_mutex_unlock(&service->commit_lock);
}
    ...
    struct wvm_v1_memory_commit_entry *entry)
{
    ...
    pthread_mutex_lock(&service->commit_lock);
    entry->state = WVM_V1_MEMORY_COMMIT_RETRYABLE;
    pthread_mutex_unlock(&service->commit_lock);
}
...
{
    if (result == 0) {
        return WVM_V1_MEM_COMMIT_ACK_SUCCESS;
    }
    if (result == -ESTALE) {
        return WVM_V1_MEM_COMMIT_ACK_STALE_BASE_VERSION;
    }
    if (result == -ENOENT) {
        return WVM_V1_MEM_COMMIT_ACK_NOT_FOUND;
    }
    if (result == -EAGAIN || result == -ENOBUFS) {
        return WVM_V1_MEM_COMMIT_ACK_BACKPRESSURE;
    }
    return WVM_V1_MEM_COMMIT_ACK_INTERNAL_FAILURE;
}
    ...
    char *error, size_t error_len)
{
    ...
    if (status == WVM_V1_MEM_COMMIT_ACK_SUCCESS && result_version == 0) {
        status = WVM_V1_MEM_COMMIT_ACK_INTERNAL_FAILURE;
    }
    if (status != WVM_V1_MEM_COMMIT_ACK_SUCCESS) {
        result_version = 0;
...
static int service_remote_commit_ack(
    struct wvm_v1_memory_service *service,
    const struct wvm_envelope_v1 *envelope, char *error, size_t error_len)
{
    struct wvm_v1_mem_commit_ack ack;
    struct wvm_v1_memory_outgoing_commit_entry *pending;
    struct wvm_v1_mem_commit_ack previous;
    int duplicate;
    int completion_result;

    if (!service->config.complete_commit ||
        envelope->payload_bytes != WVM_V1_MEM_COMMIT_ACK_BYTES ||
        wvm_v1_mem_commit_ack_decode(envelope->payload, &ack, error,
                                     error_len) != 0 ||
        !route_destination_equal(&service->config.dispatch->local_primary,
                                 &envelope->route) ||
        envelope->origin_physical_node_id !=
    ...
    if (pending->state == WVM_V1_MEMORY_COMMIT_COMPLETED) {
     ...
     */
    pending->state = WVM_V1_MEMORY_COMMIT_COMPLETED;
    pthread_mutex_unlock(&service->outgoing_commit_lock);
    ...
    uint64_t delivery_attempt_id, char *error, size_t error_len)
{
    ...
    struct wvm_envelope_v1_route route;
    struct wvm_envelope_v1 envelope;
    uint8_t payload[WVM_V1_MEM_COMMIT_HEADER_BYTES +
                    WVM_V1_MEMORY_PAGE_BYTES];
    uint8_t payload_digest[WVM_SHA256_DIGEST_BYTES];
    ...
    if (!service || !service->initialized || !operation_id ||
        ...
        data_bytes == 0 || data_bytes > WVM_V1_MEMORY_PAGE_BYTES ||
    ...
    if (resolve_route(service, &memory->directory, &route, error, error_len) !=
        0 ||
        wvm_v1_mem_commit_encode(&commit, payload, sizeof(payload),
                                 &payload_bytes, error, error_len) != 0) {
        return -ESTALE;
    }
    wvm_envelope_v1_semantic_digest(payload, payload_bytes, payload_digest);
    ...
    envelope_from_projection(service, WVM_ENVELOPE_V1_MSG_COMMIT_DIFF,
...
static int service_remote_commit(struct wvm_v1_memory_service *service,
                                 const struct wvm_envelope_v1 *envelope,
                                 char *error, size_t error_len)
{
    struct wvm_v1_mem_commit commit;
    ...
    struct wvm_envelope_v1 response;
    uint8_t payload_digest[WVM_SHA256_DIGEST_BYTES];
    uint8_t response_payload[WVM_V1_MEM_COMMIT_ACK_BYTES];
    uint64_t result_version = 0;
    uint64_t response_attempt;
    int replay;
    int apply_result;
    ...
    if (wvm_v1_mem_commit_decode(envelope->payload, envelope->payload_bytes,
    ...
    if (replay) {
        ...
        if (result.status == WVM_V1_MEM_COMMIT_ACK_SUCCESS) {
            if (result_version == 0) {
                result.status = WVM_V1_MEM_COMMIT_ACK_INTERNAL_FAILURE;
            } else {
        ...
        if (result.status == WVM_V1_MEM_COMMIT_ACK_BACKPRESSURE) {
    ...
    response_attempt = envelope->delivery_attempt_id + 1U;
    if (wvm_v1_mem_commit_ack_envelope_build(
    ...
    uint64_t delivery_attempt_id, char *error, size_t error_len)
{
    ...
    if (memory->directory_physical_node_id ==
        ...
        return complete_local_fault(
            service, operation_id, gpa, version, WVM_V1_MEM_ACK_SUCCESS,
            service->config.local_physical_node_id,
    ...
    read.reply_destination_vnode =
        service->config.dispatch->local_primary.destination_vnode;
    if (wvm_v1_mem_read_encode(&read, payload, error, error_len) != 0) {
        if (!already_waiting) {
            pending_remove(service, operation_id);
        }
        return -EINVAL;
    }
    envelope_from_projection(service, WVM_ENVELOPE_V1_MSG_MEM_READ,
                             operation_id, delivery_attempt_id, &route, payload,
...
int wvm_v1_memory_service_dispatch(
    ...
    size_t error_len)
{
    struct wvm_v1_memory_service *service = opaque;
    ...
    case WVM_ENVELOPE_V1_MSG_COMMIT_DIFF:
        return service_remote_commit(service, envelope, error, error_len);
    case WVM_ENVELOPE_V1_MSG_MEM_COMMIT_ACK:
    ...
    const uint8_t operation_id[WVM_IDENTITY_ID_BYTES])
{
    ...
    for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
        ...
        if (entry->state != WVM_V1_MEMORY_WAITER_EMPTY &&
            memcmp(entry->operation_id, operation_id,
                   sizeof(entry->operation_id)) == 0) {
            return entry;
        }
    ...
    const uint8_t operation_id[WVM_IDENTITY_ID_BYTES])
{
    ...
    for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
        ...
        if (entry->state != WVM_V1_MEMORY_COMMIT_WAITER_EMPTY &&
            memcmp(entry->operation_id, operation_id,
                   sizeof(entry->operation_id)) == 0) {
            return entry;
        }
    ...
    const uint8_t operation_id[WVM_IDENTITY_ID_BYTES], uint64_t gpa)
{
    ...
    for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
        ...
        if (entry->state == WVM_V1_MEMORY_COMMIT_WAITER_EMPTY) {
            ...
            entry->state = WVM_V1_MEMORY_COMMIT_WAITER_WAITING;
            return entry;
        }
    }
    return NULL;
...
static void global_commit_waiter_release_locked(
    struct local_commit_waiter_entry *entry)
{
    if (!entry || entry->state == WVM_V1_MEMORY_COMMIT_WAITER_EMPTY) {
        return;
    }
    ...
    const uint8_t operation_id[WVM_IDENTITY_ID_BYTES], uint64_t gpa)
{
    ...
    for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
        ...
        if (entry->state == WVM_V1_MEMORY_WAITER_EMPTY) {
            ...
            entry->state = WVM_V1_MEMORY_WAITER_WAITING;
            return entry;
        }
    }
    return NULL;
...
{
    if (!entry || entry->state == WVM_V1_MEMORY_WAITER_EMPTY) {
        return;
    }
    pthread_cond_destroy(&entry->completed);
    memset(entry, 0, sizeof(*entry));
...
static void global_reap_expired_locked(void)
{
    ...
    for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
        ...
        if ((waiter->state == WVM_V1_MEMORY_WAITER_COMPLETED ||
             waiter->state == WVM_V1_MEMORY_WAITER_ORPHANED) &&
            retention_expired(waiter->retained_until_ms, now)) {
            global_waiter_release_locked(waiter);
        }
        if ((commit_waiter->state == WVM_V1_MEMORY_COMMIT_WAITER_COMPLETED ||
             commit_waiter->state == WVM_V1_MEMORY_COMMIT_WAITER_ORPHANED) &&
    ...
    struct wvm_v1_memory_service *service)
{
    ...
    if (service && g_global_memory_service.service == service) {
        ...
        waiter->state = WVM_V1_MEMORY_WAITER_COMPLETED;
        return;
    }
    global_waiter_release_locked(waiter);
}
    ...
    struct wvm_v1_memory_service *service)
{
    ...
    if (service && g_global_memory_service.service == service) {
        ...
        waiter->state = WVM_V1_MEMORY_COMMIT_WAITER_COMPLETED;
        return;
    }
    ...
    struct wvm_v1_memory_service *service)
{
    ...
    if (g_global_memory_service.service == service) {
        ...
        for (i = 0; i < WVM_V1_MEMORY_SERVICE_MAX_PENDING; i++) {
             ...
             */
            if (waiter->state == WVM_V1_MEMORY_WAITER_WAITING) {
                waiter->version = 0;
                waiter->status = WVM_V1_MEM_ACK_INTERNAL_FAILURE;
    ...
    uint8_t page[WVM_V1_MEMORY_PAGE_BYTES], char *error, size_t error_len)
{
    ...
    if (waiter) {
        ...
        if (waiter->state == WVM_V1_MEMORY_WAITER_COMPLETED) {
            ...
            if (waiter->status == WVM_V1_MEM_ACK_SUCCESS) {
                ack->data = page;
        ...
        if (waiter->state == WVM_V1_MEMORY_WAITER_ORPHANED) {
            waiter->state = WVM_V1_MEMORY_WAITER_WAITING;
            adopted_orphan = 1;
        }
    } else {
    ...
    service_result = wvm_v1_memory_service_request_fault(
    ...
    pthread_mutex_lock(&g_global_memory_service.lock);
    while (waiter->state == WVM_V1_MEMORY_WAITER_WAITING) {
    ...
    const uint8_t *data, size_t data_bytes, char *error, size_t error_len)
{
    ...
    if (!operation_id || bytes_are_zero(operation_id, WVM_IDENTITY_ID_BYTES) ||
        status > WVM_V1_MEM_ACK_INTERNAL_FAILURE ||
        (status == WVM_V1_MEM_ACK_SUCCESS &&
         (version == 0 || !data || data_bytes != WVM_V1_MEMORY_PAGE_BYTES)) ||
        (status != WVM_V1_MEM_ACK_SUCCESS &&
    ...
    char *error, size_t error_len)
{
    ...
    if (waiter) {
        ...
        if (waiter->state == WVM_V1_MEMORY_COMMIT_WAITER_COMPLETED) {
            *ack = waiter->ack;
            pthread_mutex_unlock(&g_global_memory_service.lock);
            return 0;
        }
        if (waiter->state == WVM_V1_MEMORY_COMMIT_WAITER_WAITING) {
            pthread_mutex_unlock(&g_global_memory_service.lock);
            set_error(error, error_len,
                      "V1 local commit operation is already being waited");
            return -EALREADY;
        }
        if (waiter->state == WVM_V1_MEMORY_COMMIT_WAITER_ORPHANED) {
            waiter->state = WVM_V1_MEMORY_COMMIT_WAITER_WAITING;
            adopted_orphan = 1;
        }
    } else {
    ...
    service_result = wvm_v1_memory_service_request_commit(
        service, gpa, base_version, offset, data, data_bytes, operation_id,
        delivery_attempt_id, error, error_len);
    if (service_result != 0) {
        pthread_mutex_lock(&g_global_memory_service.lock);
        if (adopted_orphan) {
            waiter->state = WVM_V1_MEMORY_COMMIT_WAITER_ORPHANED;
            waiter->retained_until_ms = retention_deadline_ms(service);
        } else {
            global_commit_waiter_release_locked(waiter);
        }
    ...
    pthread_mutex_lock(&g_global_memory_service.lock);
    while (waiter->state == WVM_V1_MEMORY_COMMIT_WAITER_WAITING) {
        wait_result = pthread_cond_timedwait(&waiter->completed,
                                             &g_global_memory_service.lock,
                                             &deadline);
        if (wait_result == ETIMEDOUT) {
    ...
    char *error, size_t error_len)
{
    ...
    if (!operation_id || bytes_are_zero(operation_id, WVM_IDENTITY_ID_BYTES) ||
        status > WVM_V1_MEM_COMMIT_ACK_INTERNAL_FAILURE ||
        (status == WVM_V1_MEM_COMMIT_ACK_SUCCESS && result_version == 0) ||
        (status != WVM_V1_MEM_COMMIT_ACK_SUCCESS && result_version != 0)) {
