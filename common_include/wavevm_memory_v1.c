...
static int reply_destination_valid(uint16_t kind, uint64_t scope,
                                   uint32_t vnode)
{
    if (vnode == WVM_ENVELOPE_V1_ROUTE_DESTINATION_UNSPECIFIED) {
        return 0;
    }
    return (kind == WVM_ENVELOPE_V1_ROUTE_DESTINATION_FLAT_VNODE &&
            scope == 0) ||
           (kind == WVM_ENVELOPE_V1_ROUTE_DESTINATION_FRACTAL_VNODE &&
            scope != 0);
}

static int gpa_valid(uint64_t gpa)
{
    return (gpa & (WVM_V1_MEMORY_PAGE_BYTES - 1U)) == 0;
}

static int ack_status_valid(uint16_t status)
{
    return status <= WVM_V1_MEM_ACK_INTERNAL_FAILURE;
}

static int commit_ack_status_valid(uint16_t status)
{
    return status <= WVM_V1_MEM_COMMIT_ACK_INTERNAL_FAILURE;
}

static int commit_valid(const struct wvm_v1_mem_commit *commit)
{
    return commit && gpa_valid(commit->gpa) && commit->base_version != 0 &&
           commit->size != 0 && commit->size <= WVM_V1_MEMORY_PAGE_BYTES &&
           commit->offset <= WVM_V1_MEMORY_PAGE_BYTES - commit->size &&
...
int wvm_v1_mem_read_decode(const uint8_t *input, size_t input_bytes,
                           ...
                           size_t error_len)
{
    if (!input || !read || input_bytes != WVM_V1_MEM_READ_PAYLOAD_BYTES ||
                             ...
                             size_t error_len)
{
    ...
    required_bytes = WVM_V1_MEM_COMMIT_HEADER_BYTES + commit->size;
    ...
    memcpy(output + WVM_V1_MEM_COMMIT_HEADER_BYTES, commit->data,
           commit->size);
                             ...
                             size_t error_len)
{
    ...
    if (!input || !commit || input_bytes < WVM_V1_MEM_COMMIT_HEADER_BYTES ||
    ...
    if (size == 0 ||
        input_bytes != WVM_V1_MEM_COMMIT_HEADER_BYTES + (size_t)size) {
    ...
    commit->data = input + WVM_V1_MEM_COMMIT_HEADER_BYTES;
                          ...
                          char *error, size_t error_len)
{
    ...
    required_bytes = WVM_V1_MEM_ACK_HEADER_BYTES;
    if (ack->status == WVM_V1_MEM_ACK_SUCCESS) {
        ...
        required_bytes += WVM_V1_MEMORY_PAGE_BYTES;
    ...
    if (ack->status == WVM_V1_MEM_ACK_SUCCESS) {
        memcpy(output + WVM_V1_MEM_ACK_HEADER_BYTES, ack->data,
                          ...
                          size_t error_len)
{
    ...
    if (!input || !ack || input_bytes < WVM_V1_MEM_ACK_HEADER_BYTES ||
    ...
    if (status == WVM_V1_MEM_ACK_SUCCESS) {
        if (ack->version == 0 ||
            input_bytes != WVM_V1_MEM_ACK_HEADER_BYTES +
                               WVM_V1_MEMORY_PAGE_BYTES) {
            set_error(error, error_len,
                      "successful V1 memory ACK lacks a full page");
            return -1;
        }
        ack->data = input + WVM_V1_MEM_ACK_HEADER_BYTES;
        ack->data_bytes = WVM_V1_MEMORY_PAGE_BYTES;
    ...
    size_t error_len)
{
    if (!ack || !output || !gpa_valid(ack->gpa) ||
        ...
        ack->directory_physical_node_id == 0 ||
        ack->directory_node_instance_id == 0 ||
        (ack->status == WVM_V1_MEM_COMMIT_ACK_SUCCESS
    ...
    char *error, size_t error_len)
{
    ...
    if (!request || !resolved_reply_route || !ack || !payload_output ||
        ...
        wvm_v1_mem_read_decode(request->payload, request->payload_bytes,
        ...
        resolved_reply_route->destination_vnode_or_endpoint !=
            read.reply_destination_vnode ||
        ack->gpa != read.gpa ||
        wvm_v1_mem_ack_encode(ack, payload_output, payload_output_capacity,
    ...
    struct wvm_envelope_v1 *response, char *error, size_t error_len)
{
    ...
    if (!request || !resolved_reply_route || !ack || !payload_output ||
        !response || request->message_type != WVM_ENVELOPE_V1_MSG_COMMIT_DIFF ||
        response_delivery_attempt_id == 0 ||
        wvm_v1_mem_commit_decode(request->payload, request->payload_bytes,
        ...
        resolved_reply_route->destination_vnode_or_endpoint !=
            commit.reply_destination_vnode ||
        ack->gpa != commit.gpa ||
        wvm_v1_mem_commit_ack_encode(ack, payload_output, error,
                                     error_len) != 0) {
        if (!error || error[0] == '\0') {
            set_error(error, error_len,
    ...
    memset(response, 0, sizeof(*response));
    response->message_type = WVM_ENVELOPE_V1_MSG_MEM_COMMIT_ACK;
    response->vm_id = request->vm_id;
    response->vm_incarnation = request->vm_incarnation;
    response->manifest_generation = request->manifest_generation;
    response->origin_physical_node_id = request->origin_physical_node_id;
    ...
    response->route = *resolved_reply_route;
    response->payload = payload_output;
    response->payload_bytes = WVM_V1_MEM_COMMIT_ACK_BYTES;
    return 0;
}
                                   ...
                                   size_t error_len)
{
    ...
    case WVM_ENVELOPE_V1_MSG_MEM_READ:
        return wvm_v1_mem_read_decode(payload, payload_bytes, &read, error,
                                      error_len);
    case WVM_ENVELOPE_V1_MSG_MEM_ACK:
        return wvm_v1_mem_ack_decode(payload, payload_bytes, &ack, error,
                                     error_len);
    case WVM_ENVELOPE_V1_MSG_COMMIT_DIFF:
        return wvm_v1_mem_commit_decode(payload, payload_bytes, &commit, error,
                                        error_len);
    case WVM_ENVELOPE_V1_MSG_MEM_COMMIT_ACK:
        if (payload_bytes != WVM_V1_MEM_COMMIT_ACK_BYTES) {
            set_error(error, error_len, "V1 memory commit ACK has bad length");
            return -1;
        }
        return wvm_v1_mem_commit_ack_decode(payload, &commit_ack, error,
                                            error_len);
    default:
        return 0;
    }
