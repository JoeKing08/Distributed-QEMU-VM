    ...
    size_t error_len)
{
    if (!request || !output ||
        ...
        request->gpa % WVM_V1_MEMORY_PAGE_BYTES != 0) {
...
{
    if (!input || !request ||
        input_bytes != WVM_LOCAL_MEMORY_V1_FAULT_REQUEST_BYTES ||
        all_zero(input, WVM_IDENTITY_ID_BYTES) ||
        read_be64(input + 16) == 0 ||
        read_be64(input + 24) % WVM_V1_MEMORY_PAGE_BYTES != 0) {
        set_error(error, error_len, "local V1 memory fault request is invalid");
        return -1;
    }
    ...
    char *error, size_t error_len)
{
    ...
    if (!request || !output || !output_bytes ||
        ...
        output_capacity < WVM_IDENTITY_ID_BYTES + 8U +
                              WVM_V1_MEM_COMMIT_HEADER_BYTES ||
    ...
    size_t error_len)
{
    if (!output ||
        (ack_payload_bytes != 0 &&
         ack_payload_bytes != WVM_V1_MEM_ACK_HEADER_BYTES &&
         ack_payload_bytes !=
    ...
    size_t *ack_payload_bytes, char *error, size_t error_len)
{
    ...
    if (decoded != 0 && decoded != WVM_V1_MEM_ACK_HEADER_BYTES &&
        decoded != WVM_V1_MEM_ACK_HEADER_BYTES + WVM_V1_MEMORY_PAGE_BYTES) {
        set_error(error, error_len, "local V1 memory result length is invalid");
        return -1;
    }
