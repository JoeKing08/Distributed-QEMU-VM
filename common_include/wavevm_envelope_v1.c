...
uint32_t wvm_envelope_v1_crc32c(const uint8_t *bytes, size_t byte_count)
{
                                      ...
                                      size_t payload_bytes)
{
    ...
    pthread_once(&crc32c_once, crc32c_init);
    crc = crc32c_update(crc, header, WVM_ENVELOPE_V1_HEADER_BYTES - 4U);
    crc = crc32c_update(crc, payload, payload_bytes);
    return crc ^ 0xffffffffU;
}

void wvm_envelope_v1_semantic_digest(
...
static int message_type_known(uint16_t message_type)
{
    ...
    case WVM_ENVELOPE_V1_MSG_MEM_WRITE:
    ...
    case WVM_ENVELOPE_V1_MSG_INVALIDATE:
    case WVM_ENVELOPE_V1_MSG_DOWNGRADE:
    case WVM_ENVELOPE_V1_MSG_FETCH_AND_INVALIDATE:
    case WVM_ENVELOPE_V1_MSG_WRITE_BACK:
    case WVM_ENVELOPE_V1_MSG_DECLARE_INTEREST:
    case WVM_ENVELOPE_V1_MSG_PAGE_PUSH_FULL:
    case WVM_ENVELOPE_V1_MSG_PAGE_PUSH_DIFF:
    case WVM_ENVELOPE_V1_MSG_COMMIT_DIFF:
    case WVM_ENVELOPE_V1_MSG_FORCE_SYNC:
    case WVM_ENVELOPE_V1_MSG_MEM_COMMIT_ACK:
    case WVM_ENVELOPE_V1_MSG_BLOCK_READ:
    case WVM_ENVELOPE_V1_MSG_BLOCK_WRITE:
    case WVM_ENVELOPE_V1_MSG_BLOCK_ACK:
    case WVM_ENVELOPE_V1_MSG_BLOCK_FLUSH:
    case WVM_ENVELOPE_V1_MSG_CTRL_RESULT:
    case WVM_ENVELOPE_V1_MSG_REGISTER_MEMBER:
    case WVM_ENVELOPE_V1_MSG_CORDON:
    case WVM_ENVELOPE_V1_MSG_DRAIN:
    case WVM_ENVELOPE_V1_MSG_PREPARE_RESERVATION:
    case WVM_ENVELOPE_V1_MSG_COMMIT_RESERVATION:
    case WVM_ENVELOPE_V1_MSG_ABORT_RESERVATION:
    case WVM_ENVELOPE_V1_MSG_PREPARE_MANIFEST:
    case WVM_ENVELOPE_V1_MSG_ACTIVATE_MANIFEST:
    case WVM_ENVELOPE_V1_MSG_ABORT_MANIFEST:
    case WVM_ENVELOPE_V1_MSG_QUERY_TX:
    case WVM_ENVELOPE_V1_MSG_ROUTE_PREPARE:
    case WVM_ENVELOPE_V1_MSG_ROUTE_COMMIT:
    case WVM_ENVELOPE_V1_MSG_ROUTE_RETIRE:
    case WVM_ENVELOPE_V1_MSG_REJOIN:
    case WVM_ENVELOPE_V1_MSG_RECOVERY_REBIND:
        return 1;
    default:
        return 0;
    }
...
static int route_prefix_required(const struct wvm_envelope_v1 *envelope)
{
    return envelope && routed_data_message(envelope->message_type);
}

static int route_fields_valid(const struct wvm_envelope_v1 *envelope,
                              char *error, size_t error_len)
{
    ...
    case WVM_ENVELOPE_V1_ROUTE_DESTINATION_FLAT_VNODE:
        ...
        return 0;
    case WVM_ENVELOPE_V1_ROUTE_DESTINATION_FRACTAL_VNODE:
        if (route->destination_scope == 0) {
            set_error(error, error_len,
...
static size_t route_prefix_bytes(const struct wvm_envelope_v1 *envelope)
{
    return route_prefix_required(envelope) ? WVM_ENVELOPE_V1_ROUTE_PREFIX_BYTES
                                           : 0U;
}

static int parse_fragment_prefix(const struct wvm_envelope_v1 *envelope,
                                 ...
                                 size_t error_len)
{
    ...
    if (!envelope || !prefix ||
        envelope->payload_bytes < WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES) {
    ...
    prefix->data = envelope->payload + WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES;
    prefix->data_bytes =
        envelope->payload_bytes - WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES;
    for (i = 12; i < WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES; i++) {
    ...
    if (prefix->logical_payload_bytes == 0 ||
        prefix->logical_payload_bytes >
            WVM_ENVELOPE_V1_MAX_NETWORK_LOGICAL_PAYLOAD ||
        prefix->fragment_count == 0 ||
        prefix->fragment_count > WVM_ENVELOPE_V1_MAX_FRAGMENTS ||
        prefix->fragment_index >= prefix->fragment_count ||
        prefix->data_bytes == 0 ||
        prefix->data_bytes > WVM_ENVELOPE_V1_MAX_FRAGMENT_DATA_BYTES ||
...
static int transport_payload_valid(const struct wvm_envelope_v1 *envelope,
                                   enum wvm_envelope_v1_transport transport,
                                   char *error, size_t error_len)
{
    ...
    if (transport == WVM_ENVELOPE_V1_TRANSPORT_LOCAL) {
        if (frame_payload_bytes > WVM_ENVELOPE_V1_MAX_LOCAL_PAYLOAD) {
            set_error(error, error_len, "local envelope payload exceeds limit");
            return -1;
        }
        return 0;
    }
    if (transport != WVM_ENVELOPE_V1_TRANSPORT_NETWORK) {
        set_error(error, error_len, "unknown envelope transport");
        return -1;
    }
    max_frame_payload = WVM_ENVELOPE_V1_MAX_NETWORK_FRAME_BYTES -
                        WVM_ENVELOPE_V1_HEADER_BYTES;
...
static int envelope_fields_valid(const struct wvm_envelope_v1 *envelope,
                                 enum wvm_envelope_v1_transport transport,
                                 ...
                                 size_t error_len)
{
    ...
    if (!envelope || !message_type_known(envelope->message_type) ||
        (envelope->flags & ~WVM_ENVELOPE_V1_KNOWN_FLAGS) != 0 ||
    ...
    if (verify_semantic_digest) {
        wvm_envelope_v1_semantic_digest(envelope->payload,
...
int wvm_envelope_v1_encode(
    const struct wvm_envelope_v1 *envelope,
    enum wvm_envelope_v1_transport transport, uint8_t *output,
    size_t output_capacity, size_t *encoded_bytes, char *error,
    size_t error_len)
{
    struct wvm_envelope_v1 normalized;
    ...
    if (!envelope || !output || !encoded_bytes ||
        ...
        envelope->payload_bytes >
            SIZE_MAX - WVM_ENVELOPE_V1_HEADER_BYTES) {
        set_error(error, error_len, "envelope encode input is invalid");
        return -1;
    }
    normalized = *envelope;
    if (!(normalized.flags & WVM_ENVELOPE_V1_FLAG_FRAGMENTED)) {
        wvm_envelope_v1_semantic_digest(normalized.payload,
    ...
    total_bytes = WVM_ENVELOPE_V1_HEADER_BYTES + frame_payload_bytes;
    if (output_capacity < total_bytes) {
        set_error(error, error_len, "envelope output buffer is too small");
        return -1;
    }

    memset(output, 0, WVM_ENVELOPE_V1_HEADER_BYTES);
    write_be32(output + 0, WVM_ENVELOPE_V1_MAGIC);
    write_be16(output + 4, WVM_ENVELOPE_V1_PROTOCOL_VERSION);
    write_be16(output + 6, WVM_ENVELOPE_V1_HEADER_BYTES);
    ...
    if (routing_bytes != 0) {
        uint8_t *prefix = output + WVM_ENVELOPE_V1_HEADER_BYTES;

        write_be16(prefix + 0, WVM_ENVELOPE_V1_ROUTE_PREFIX_VERSION);
        write_be16(prefix + 2, WVM_ENVELOPE_V1_ROUTE_PREFIX_BYTES);
    ...
    if (normalized.payload_bytes != 0) {
        memcpy(output + WVM_ENVELOPE_V1_HEADER_BYTES + routing_bytes,
               normalized.payload,
               normalized.payload_bytes);
    }
    write_be32(output + 160,
               crc32c_header_payload(output,
                                     output + WVM_ENVELOPE_V1_HEADER_BYTES,
                                     frame_payload_bytes));
    *encoded_bytes = total_bytes;
    return 0;
}

int wvm_envelope_v1_emit_network_frames(
    const struct wvm_envelope_v1 *envelope,
    wvm_envelope_v1_emit_frame_fn emit_frame, void *opaque, char *error,
    size_t error_len)
{
    struct wvm_envelope_v1 fragment;
    uint8_t frame[WVM_ENVELOPE_V1_MAX_NETWORK_FRAME_BYTES];
    uint8_t fragment_payload[WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES +
    ...
    if (!envelope || !emit_frame ||
        (envelope->payload_bytes != 0 && !envelope->payload) ||
        (envelope->flags & WVM_ENVELOPE_V1_FLAG_FRAGMENTED) != 0 ||
        envelope->payload_bytes > WVM_ENVELOPE_V1_MAX_NETWORK_LOGICAL_PAYLOAD) {
        set_error(error, error_len,
    ...
    max_unfragmented_payload =
        WVM_ENVELOPE_V1_MAX_NETWORK_FRAME_BYTES -
        WVM_ENVELOPE_V1_HEADER_BYTES - route_bytes;
    if (envelope->payload_bytes <= max_unfragmented_payload) {
        if (wvm_envelope_v1_encode(envelope, WVM_ENVELOPE_V1_TRANSPORT_NETWORK,
    ...
    fragment_count =
        (uint16_t)((envelope->payload_bytes +
                    WVM_ENVELOPE_V1_MAX_FRAGMENT_DATA_BYTES - 1U) /
                   WVM_ENVELOPE_V1_MAX_FRAGMENT_DATA_BYTES);
    if (fragment_count == 0 ||
        fragment_count > WVM_ENVELOPE_V1_MAX_FRAGMENTS) {
    ...
    for (fragment_index = 0; fragment_index < fragment_count;
        ...
        if (data_bytes > WVM_ENVELOPE_V1_MAX_FRAGMENT_DATA_BYTES) {
            data_bytes = WVM_ENVELOPE_V1_MAX_FRAGMENT_DATA_BYTES;
        }
        memset(fragment_payload, 0,
               WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES + data_bytes);
        ...
        memcpy(fragment_payload + WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES,
               envelope->payload + offset, data_bytes);

        fragment = *envelope;
        fragment.flags |= WVM_ENVELOPE_V1_FLAG_FRAGMENTED;
        ...
        fragment.payload_bytes =
            WVM_ENVELOPE_V1_FRAGMENT_PREFIX_BYTES + data_bytes;
        if (wvm_envelope_v1_encode(&fragment,
...
int wvm_envelope_v1_decode(
    const uint8_t *input, size_t input_bytes,
    enum wvm_envelope_v1_transport transport,
    struct wvm_envelope_v1 *envelope, char *error, size_t error_len)
{
    ...
    if (!input || !envelope || input_bytes < WVM_ENVELOPE_V1_HEADER_BYTES ||
        read_be32(input + 0) != WVM_ENVELOPE_V1_MAGIC ||
        read_be16(input + 4) != WVM_ENVELOPE_V1_PROTOCOL_VERSION ||
        read_be16(input + 6) != WVM_ENVELOPE_V1_HEADER_BYTES) {
        set_error(error, error_len, "envelope prefix is invalid");
        return -1;
    }
    payload_bytes = read_be32(input + 12);
    if (input_bytes - WVM_ENVELOPE_V1_HEADER_BYTES != payload_bytes) {
        set_error(error, error_len, "envelope length is invalid");
        return -1;
    }
    received_crc = read_be32(input + 160);
    if (received_crc !=
        crc32c_header_payload(input, input + WVM_ENVELOPE_V1_HEADER_BYTES,
                              payload_bytes)) {
    ...
    if (routing_bytes != 0) {
        const uint8_t *prefix = input + WVM_ENVELOPE_V1_HEADER_BYTES;

        if (read_be16(prefix + 0) != WVM_ENVELOPE_V1_ROUTE_PREFIX_VERSION ||
            read_be16(prefix + 2) != WVM_ENVELOPE_V1_ROUTE_PREFIX_BYTES ||
    ...
    envelope->payload = input + WVM_ENVELOPE_V1_HEADER_BYTES + routing_bytes;
    return envelope_fields_valid(envelope, transport, 1, error, error_len);
}

int wvm_envelope_v1_validate_admitted(
    const struct wvm_envelope_v1 *envelope,
    const struct wvm_envelope_v1_admitted_identity *identity, char *error,
    size_t error_len)
{
...
int wvm_envelope_v1_route_advance(struct wvm_envelope_v1 *envelope,
                                  char *error, size_t error_len)
{
...
int wvm_envelope_v1_reassembler_init(
    struct wvm_envelope_v1_reassembler *reassembler)
{
    ...
    entries = calloc(WVM_ENVELOPE_V1_MAX_REASSEMBLIES, sizeof(*entries));
    if (!entries) {
        return -1;
    }
    reassembler->entries = entries;
    reassembler->entry_capacity = WVM_ENVELOPE_V1_MAX_REASSEMBLIES;
    return 0;
}

void wvm_envelope_v1_reassembler_destroy(
    struct wvm_envelope_v1_reassembler *reassembler)
{
    struct reassembly_entry *entries;
    size_t i;
...
static int operation_key_equal(const struct wvm_envelope_v1 *left,
                               const struct wvm_envelope_v1 *right)
{
...
static void reassembler_expire(struct wvm_envelope_v1_reassembler *reassembler,
                               uint64_t now_ms)
{
...
static struct reassembly_entry *reassembly_find(
    struct wvm_envelope_v1_reassembler *reassembler,
    const struct wvm_envelope_v1 *fragment)
{
    struct reassembly_entry *entries = reassembler->entries;
    size_t i;
    ...
    size_t error_len)
{
    ...
    if (prefix->logical_payload_bytes >
        WVM_ENVELOPE_V1_MAX_REASSEMBLY_BYTES -
            reassembler->allocated_payload_bytes) {
    ...
    entry->in_use = 1;
    entry->expiry_ms = now_ms + WVM_ENVELOPE_V1_REASSEMBLY_LIFETIME_MS;
...
int wvm_envelope_v1_reassembler_accept(
    struct wvm_envelope_v1_reassembler *reassembler,
    const struct wvm_envelope_v1 *fragment, uint64_t now_ms,
    struct wvm_envelope_v1_reassembled *output, char *error,
    size_t error_len)
{
    struct fragment_prefix prefix;
    ...
    wvm_envelope_v1_semantic_digest(entry->payload,
    ...
    output->envelope.flags &= ~WVM_ENVELOPE_V1_FLAG_FRAGMENTED;
...
void wvm_envelope_v1_reassembled_release(
